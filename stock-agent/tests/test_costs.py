# Checked against hand-computed values rather than the implementation: a cost model that is
# merely self-consistent is the easiest way to produce a backtest wrong in the flattering direction.
...
def test_unknown_volatility_is_charged_pessimistically():
    # Not symmetry for its own sake. ADV and daily vol come from the same bars over the same window,
    # so whenever one is missing the other is too — and a missing vol that zeroed impact would make
    # the pessimistic ADV fallback unreachable in the engine.
