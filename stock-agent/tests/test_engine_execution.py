# What the engine may know at the open, what it does when it cannot trade, and what it refuses to
# run. These cover the two P1 defects in the loop; both produced complete, plausible equity curves,
# which is why neither showed up in the existing suite.
...
def second_execution_day(dates: pd.DatetimeIndex) -> pd.Timestamp:
    # The first execution day is uninformative: the portfolio is all cash, so NAV does not depend on
    # any holding's mark and the defect under test cannot show.
...
def halted_panel(gap_close=None):
    # `gap_close` overrides BBB's *closing* price on the same day it is missing its open. The pair
    # matters because nothing knowable at the open differs between the two variants.
...
def test_an_execution_days_close_cannot_change_that_mornings_orders():
    # A missing open used to fall back to that day's close, which is not knowable at the open, so a
    # halted name's evening price set the NAV every other name was sized against: moving one close
    # from 100 to 50 turned another name's morning order from nothing into a 125-share sale.
    orders = {}
...
def test_a_blocked_trade_is_carried_to_the_next_session():
    # Dropped silently, a name the strategy wanted stayed unbought — or one it had dropped stayed
    # held — until the next scheduled rebalance, with nothing in the output saying so.
...
def test_deferring_does_not_rebalance_the_rest_of_the_book():
    # Re-planning the whole target would drag every position that had since drifted back through the
    # band, turning one halted name into a portfolio-wide trade on an unscheduled day.
    panel, execution_day = halted_panel()
    next_session = panel.dates[panel.dates.get_loc(execution_day) + 1]
...
def test_a_minimum_commission_cannot_push_cash_negative():
    # A fixed minimum fee does not shrink with the order: the final fill recomputes the same flat
    # charge against a smaller notional and overruns the cash reserved for it. Measured before the
    # fix, this exact setup finished at -0.50.
...
def test_nonsensical_settings_are_refused(flat_panel, bad):
    # NaN is the one worth stating: every comparison against it is False, so the limit is simply
    # not enforced and the run returns a plausible curve computed without it.
...
def test_excluded_names_are_disclosed_on_the_result(flat_panel):
    # The exclusions used to be printed by the loader and forgotten, leaving the console the only
    # record that the universe had changed under the result.
