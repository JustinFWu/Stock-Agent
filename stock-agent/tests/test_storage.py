# Every cache here is something the next run depends on. A write interrupted partway used to leave
# a truncated file where a good one was expected, and a partial parquet or JSON is often still
# parseable as *something*, so the failure arrives later looking like a data problem.
...
def test_a_failed_write_leaves_the_previous_artifact_intact(tmp_path):
    # The behaviour that matters: a failed rebuild costs you the rebuild, not the artifact you
    # already had. Writing in place costs you both.
    destination = tmp_path / "artifact.txt"
...
def test_the_temporary_sits_beside_its_destination(tmp_path):
    # A temporary in the system temp directory turns the rename into a copy, which is precisely the
    # non-atomic operation being avoided.
